<footer class="pt-14 pb-6">
  <div class="max-w-[1320px] mx-auto px-[6vw]">
    <div class="grid grid-cols-1 md:grid-cols-[1.4fr_0.8fr_0.8fr_0.8fr] gap-8 pb-8 border-b border-line">
      <div>
        <a href="{{ route('home') }}" class="font-serif text-xl tracking-wide flex items-center gap-2.5 mb-3">
        ...
        @if($siteSetting?->hero_tagline)
          <p class="text-ivory-dim font-light text-sm max-w-[280px]">{{ $siteSetting->hero_tagline }}</p>
        @endif
      </div>
      <div>
      ...
      <div>
        ...
        @if($siteSetting->contact_email)<p class="text-ivory-dim text-sm font-light mb-3">{{ $siteSetting->contact_email }}</p>@endif
        @if($siteSetting->contact_phone)<p class="text-ivory-dim text-sm font-light mb-3">{{ $siteSetting->contact_phone }}</p>@endif
        @if($siteSetting->address)<p class="text-ivory-dim text-sm font-light mb-3">{{ $siteSetting->address }}</p>@endif
      </div>
      @endif
      @if($siteSetting?->instagram_url || $siteSetting?->facebook_url || $siteSetting?->youtube_url || $siteSetting?->whatsapp_number)
      <div>
        <h5 class="font-mono text-[0.7rem] tracking-[0.16em] uppercase text-silver-dim mb-3">Follow</h5>
        @if($siteSetting->instagram_url)<a href="{{ $siteSetting->instagram_url }}" target="_blank" rel="noopener" class="block text-ivory-dim text-sm font-light mb-2 hover:text-gold-bright transition-colors">Instagram</a>@endif
        @if($siteSetting->facebook_url)<a href="{{ $siteSetting->facebook_url }}" target="_blank" rel="noopener" class="block text-ivory-dim text-sm font-light mb-2 hover:text-gold-bright transition-colors">Facebook</a>@endif
        @if($siteSetting->youtube_url)<a href="{{ $siteSetting->youtube_url }}" target="_blank" rel="noopener" class="block text-ivory-dim text-sm font-light mb-2 hover:text-gold-bright transition-colors">YouTube</a>@endif
        @if($siteSetting->whatsapp_number)<a href="https://wa.me/{{ preg_replace('/\D/', '', $siteSetting->whatsapp_number) }}" target="_blank" rel="noopener" class="block text-ivory-dim text-sm font-light mb-2 hover:text-gold-bright transition-colors">WhatsApp</a>@endif
      </div>
      @endif
    </div>
    <div class="flex justify-between items-center text-xs text-silver-dim flex-wrap gap-3.5 pt-5">
      <span>© {{ date('Y') }} {{ $siteSetting->studio_name ?? 'Waka Shots' }}. All rights reserved.</span>
